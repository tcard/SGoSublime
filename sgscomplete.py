...
class SGoSublime(sublime_plugin.EventListener):
	gocode_set = False
	def on_query_completions(self, view, prefix, locations):
		pos = locations[0]
		scopes = view.scope_name(pos).split()
		if ('source.sgo' not in scopes) or (gs.setting('gscomplete_enabled', False) is not True):
			return []
		...
		types = []
		for r in view.find_by_selector('source.sgo keyword.'):
		...
		try:
			if basename(view.file_name()) == "main.sgo":
		...
		if show_snippets and (pc.isspace() or pc.isalpha()):
			if scopes[-1] == 'source.sgo':
				cl.extend(resolve_snippets(ctx))
			elif scopes[-1] == 'meta.block.sgo' and ('meta.function.plain.sgo' in scopes or 'meta.function.receiver.sgo' in scopes):
				ctx['global'] = False
				ctx['local'] = True
				cl.extend(resolve_snippets(ctx))
		return (cl, AC_OPTS)
