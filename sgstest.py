	...
	def run(self):
		def f(res, err):
			...
			def cb(i, win):
				if i >= 0:
					a = args.get(ents[i], [])
					win.active_view().run_command('gs9o_open', {'run': gs.lst('sgo', 'test', a)})
...
def handle_action(view, action):
	...
	ok = prefix and fn and fn.endswith('_test.sgo')
	if ok:
		...
		if prefix == 'Benchmark':
			cmd = ['sgo', 'test', '-test.run=none', '-test.bench="%s"' % pat]
		else:
			cmd = ['sgo', 'test', '-test.run="%s"' % pat]
