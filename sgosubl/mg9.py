...
def gs_init(m={}):
	...
	gsq.do('SGoSublime', f, msg='Installing MarGo', set_status=False)
...
def install(aso_install_vesion, force_install):
	...
	if _inst_state() != "":
		gs.notify(DOMAIN, 'Installation aborted. Install command already called for SGoSublime %s.' % INSTALL_VERSION)
		return
	...
	if not is_update and not force_install and _bins_exist() and aso_install_vesion == INSTALL_VERSION:
		...
		gs.notify('SGoSublime', 'Installing MarGo')
		start = time.time()

		gopath = gs.dist_path()
		env_gopath = sh.env().get('GOPATH')
		if env_gopath:
			gopath += ':' + env_gopath
		...
		if cr.ok and _bins_exist():
			...
			gs.show_output('SGoSublime', '\n'.join(sanity_check_sl(sl)))

	gs.set_attr(_inst_name(), 'done')

	if is_update:
		gs.show_output('SGoSublime-source', '\n'.join([
			'SGoSublime source has been updated.',
		...
		e = sh.env()
		a = [
			'SGoSublime init %s (%0.3fs)' % (INSTALL_VERSION, time.time() - init_start),
		]
		...
		report_x = lambda: gs.println("SGoSublime: Exception while cleaning up old binaries", gs.traceback())
		try:
			bin_dirs = [
				gs.home_path('bin'),
			]
			...
			for fn in l:
				try:
					gs.println("SGoSublime: removing old binary: `%s'" % fn)
					os.remove(fn)
				except Exception:
					report_x()
			...
			try:
				...
				if ln:
					...
					if req and req.f:
						if tag != TAG:
							gs.notice(DOMAIN, "\n".join([
								"SGoSublime/MarGo appears to be out-of-sync.",
