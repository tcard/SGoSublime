...
IGNORED_SCOPES = frozenset([
	'string.quoted.double.sgo',
	'string.quoted.single.sgo',
	'string.quoted.raw.sgo',
	'comment.line.double-slash.sgo',
	'comment.block.sgo',
...
def settings_obj():
	return sublime.load_settings("SGoSublime.sublime-settings")

def aso():
	return sublime.load_settings("SGoSublime-aux.sublime-settings")

def save_aso():
	return sublime.save_settings("SGoSublime-aux.sublime-settings")
...
def is_go_source_view(view=None, strict=True):
	...
	selector_match = view.score_selector(sel(view).begin(), 'source.sgo') > 0
	if selector_match:
		return True
...
def traceback(domain='SGoSublime'):
...
def tm_path(name):
	d = {
		'sgo': 'syntax/SGoSublime-SGo.tmLanguage',
	}

	try:
		so = sublime.load_settings('SGoSublime-next.sublime-settings')
		if 'sgo' in so.get('extensions', []):
			d['sgo'] = 'SGoSublime-next.tmLanguage'
	except Exception:
		pass

	return 'Packages/SGoSublime/%s' % d[name]
...
def dist_path(*a):
	return os.path.join(packages_dir(), 'SGoSublime', *a)
...
def _home_path(*a):
	return os.path.join(packages_dir(), 'User', 'SGoSublime', about.PLATFORM, *a)
...
def gs_init(m={}):
	...
	settings_obj().clear_on_change("SGoSublime.settings")
	settings_obj().add_on_change("SGoSublime.settings", sync_settings)
	sync_settings()
